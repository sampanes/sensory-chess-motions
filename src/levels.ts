import { Level } from './types';

export const levels: Level[] = [
  {
    name: 'The Royal Path',
    description: 'The Queen can move any way she likes!',
    pieceType: 'queen',
    start: { row: 4, col: 0 },
    goal: { row: 0, col: 4 },
    obstacles: {
      fences: [],
      rivers: [],
      bridges: [],
      food: [],
    },
    starThresholds: { three: 1, two: 2 },
    hint: 'A Queen can move like a Rook or a Bishop. Go straight there in one diagonal slide!',
  },
  {
    name: 'The Crown’s Detour',
    description: 'A long fence blocks the direct diagonal. Use your Rook moves!',
    pieceType: 'queen',
    start: { row: 4, col: 4 },
    goal: { row: 0, col: 0 },
    obstacles: {
      fences: [
        { row: 2, col: 2, side: 'top' },
        { row: 1, col: 1, side: 'right' },
      ],
      rivers: [],
      bridges: [],
      food: [],
    },
    starThresholds: { three: 2, two: 3 },
    hint: 'If you can’t slide diagonally through the fence, try moving straight up and then left.',
  },
  {
    name: 'The Queen’s Moat',
    description: 'The river is wide, but the Queen finds the gap.',
    pieceType: 'queen',
    start: { row: 4, col: 2 },
    goal: { row: 0, col: 2 },
    obstacles: {
      fences: [],
      rivers: [
        { row: 2, col: 0 },
        { row: 2, col: 1 },
        { row: 2, col: 2 }, // Blocked center
        { row: 2, col: 3 },
        { row: 2, col: 4 },
      ],
      bridges: [
        { row: 2, col: 0 }, // Bridge on far left
        { row: 2, col: 4 }, // Bridge on far right
      ],
      food: [],
    },
    starThresholds: { three: 3, two: 4 },
    hint: 'Use a diagonal move to reach a bridge, cross it, and slide back to the center.',
  },
  {
    name: 'The Grand Slalom',
    description: 'The Queen must weave through rivers and fences.',
    pieceType: 'queen',
    start: { row: 4, col: 0 },
    goal: { row: 0, col: 4 },
    obstacles: {
      fences: [
        { row: 0, col: 2, side: 'bottom' },
      ],
      rivers: [
        { row: 2, col: 0 },
        { row: 2, col: 1 },
        { row: 2, col: 2 },  
        { row: 2, col: 3 },
        { row: 1, col: 3 },
        { row: 1, col: 4 },
      ],
      bridges: [
        { row: 2, col: 2 },
      ],
      food: [],
    },
    starThresholds: { three: 3, two: 5 },
    hint: 'Try going straight to the bottom corner, then diagonal to clear the fences.',
  },
  {
    name: 'The Castle Road',
    description: 'Guide the Rook straight up to the flag!',
    pieceType: 'rook',
    start: { row: 4, col: 2 },
    goal: { row: 0, col: 2 },
    obstacles: {
      fences: [],
      rivers: [],
      bridges: [],
      food: [],
    },
    starThresholds: { three: 1, two: 2 },
    hint: 'The Rook moves in straight lines — up, down, left, or right.',
  },
  {
    name: 'Around the Fence',
    description: 'A fence blocks the way — go around it!',
    pieceType: 'rook',
    start: { row: 4, col: 2 },
    goal: { row: 0, col: 2 },
    obstacles: {
      fences: [{ row: 2, col: 2, side: 'top' }],
      rivers: [],
      bridges: [],
      food: [],
    },
    starThresholds: { three: 3, two: 4 },
    hint: 'Move sideways first, then slide up, then come back toward the flag.',
  },
  {
    name: 'River Crossing',
    description: 'A river blocks the path. Find the bridge!',
    pieceType: 'rook',
    start: { row: 4, col: 2 },
    goal: { row: 0, col: 2 },
    obstacles: {
      fences: [],
      rivers: [
        { row: 2, col: 0 },
        { row: 2, col: 1 },
        { row: 2, col: 2 },
        { row: 2, col: 3 },
        { row: 2, col: 4 },
      ],
      bridges: [{ row: 2, col: 4 }],
      food: [],
    },
    starThresholds: { three: 3, two: 4 },
    hint: 'Slide to the far-right bridge, cross the river, then slide back to the goal.',
  },
  {
    name: 'Fence Garden',
    description: 'Two fences make a tiny maze for the Rook.',
    pieceType: 'rook',
    start: { row: 4, col: 0 },
    goal: { row: 0, col: 4 },
    obstacles: {
      fences: [
        { row: 0, col: 0, side: 'bottom' },
        { row: 3, col: 1, side: 'top' },
        { row: 1, col: 3, side: 'left' },
        { row: 4, col: 4, side: 'left' },
      ],
      rivers: [],
      bridges: [],
      food: [],
    },
    starThresholds: { three: 3, two: 4 },
    hint: 'The Rook likes long straight roads. Find the route that lines up the goal.',
  },
  {
    name: 'Bridge Parade',
    description: 'A wider river has a bridge in the middle lane.',
    pieceType: 'rook',
    start: { row: 4, col: 4 },
    goal: { row: 0, col: 0 },
    obstacles: {
      fences: [],
      rivers: [
        { row: 2, col: 0 },
        { row: 2, col: 1 },
        { row: 2, col: 2 },
        { row: 2, col: 3 },
        { row: 2, col: 4 },
      ],
      bridges: [{ row: 2, col: 2 }],
      food: [],
    },
    starThresholds: { three: 3, two: 4 },
    hint: 'Line up with the center bridge first, then cross.',
  },
  {
    name: 'Diagonal Dash',
    description: 'The Bishop slides corner to corner.',
    pieceType: 'bishop',
    start: { row: 4, col: 0 },
    goal: { row: 0, col: 4 },
    obstacles: {
      fences: [],
      rivers: [],
      bridges: [],
      food: [],
    },
    starThresholds: { three: 1, two: 2 },
    hint: 'Bishops only move diagonally.',
  },
  {
    name: 'Zigzag Meadow',
    description: 'A pond blocks the easy diagonal path.',
    pieceType: 'bishop',
    start: { row: 4, col: 0 },
    goal: { row: 1, col: 3 },
    obstacles: {
      fences: [],
      rivers: [
        { row: 2, col: 2 },
        { row: 3, col: 3 },
      ],
      bridges: [],
      food: [],
    },
    starThresholds: { three: 4, two: 5 },
    hint: 'Take a smaller diagonal first, then change direction.',
  },
  {
    name: 'Bridge Hop',
    description: 'The Bishop can cross the stream through the bridge.',
    pieceType: 'bishop',
    start: { row: 4, col: 0 },
    goal: { row: 0, col: 0 },
    obstacles: {
      fences: [],
      rivers: [
        { row: 2, col: 0 },
        { row: 2, col: 1 },
        { row: 2, col: 2 },
        { row: 2, col: 3 },
        { row: 2, col: 4 },
      ],
      bridges: [{ row: 2, col: 2 }],
      food: [],
    },
    starThresholds: { three: 4, two: 5 },
    hint: 'Aim along a diagonal that passes through the center bridge.',
  },
  {
    name: 'Mirror Mountain',
    description: 'Bounce diagonally around little obstacles to the far side.',
    pieceType: 'bishop',
    start: { row: 4, col: 4 },
    goal: { row: 0, col: 0 },
    obstacles: {
      fences: [],
      rivers: [
        { row: 2, col: 3 },
        { row: 2, col: 2 },
        { row: 2, col: 1 },
        { row: 2, col: 0 },
      ],
      bridges: [
        { row: 2, col: 4 },
      ],
      food: [],
    },
    starThresholds: { three: 5, two: 6 },
    hint: 'If the longest diagonal is blocked, land on a different color-matching square first.',
  },
  {
    name: 'The Horse Jump',
    description: 'The Knight jumps in an L-shape.',
    pieceType: 'knight',
    start: { row: 2, col: 2 },
    goal: { row: 0, col: 1 },
    obstacles: {
      fences: [
        { row: 1, col: 3, side: 'top' },
        { row: 1, col: 3, side: 'right' },
        { row: 2, col: 3, side: 'right' },
        { row: 3, col: 3, side: 'right' },
        { row: 3, col: 3, side: 'bottom' },
        { row: 3, col: 2, side: 'bottom' },
        { row: 3, col: 1, side: 'bottom' },
        { row: 3, col: 1, side: 'left' },
      ],
      rivers: [
        { row: 0, col: 2 },
        { row: 1, col: 2 },
        { row: 1, col: 1 },
        { row: 2, col: 1 },
        { row: 2, col: 0 },
      ],
      bridges: [],
      food: [],
    },
    starThresholds: { three: 1, two: 2 },
    hint: 'Two squares one way and one square sideways makes an L.',
  },
  {
    name: 'Leaping the Fence',
    description: 'Fences do not stop a Knight.',
    pieceType: 'knight',
    start: { row: 4, col: 0 },
    goal: { row: 1, col: 3 },
    obstacles: {
      fences: [
        { row: 3, col: 0, side: 'top' },
        { row: 3, col: 1, side: 'top' },
        { row: 3, col: 2, side: 'top' },
        { row: 3, col: 3, side: 'top' },
        { row: 3, col: 4, side: 'top' },
      ],
      rivers: [],
      bridges: [],
      food: [],
    },
    starThresholds: { three: 2, two: 3 },
    hint: 'Jump over the fence line and keep aiming for the corner.',
  },
  {
    name: 'River Jumper',
    description: 'Jump over the river and land safely on the other side!',
    pieceType: 'knight',
    start: { row: 4, col: 2 },
    goal: { row: 1, col: 1 },
    obstacles: {
      fences: [],
      rivers: [
        { row: 2, col: 0 },
        { row: 2, col: 1 },
        { row: 2, col: 2 },
        { row: 2, col: 3 },
        { row: 2, col: 4 },
      ],
      bridges: [],
      food: [],
    },
    starThresholds: { three: 2, two: 3 },
    hint: 'Knights can jump over the river row. Try landing on the top half of the board first.',
  },
  {
    name: 'Bridge and Fence Picnic',
    description: 'The Knight ignores both fences and rivers while it hops.',
    pieceType: 'knight',
    start: { row: 4, col: 3 },
    goal: { row: 1, col: 0 },
    obstacles: {
      fences: [
        { row: 3, col: 2, side: 'top' },
        { row: 1, col: 1, side: 'right' },
      ],
      rivers: [
        { row: 2, col: 0 },
        { row: 2, col: 1 },
        { row: 2, col: 2 },
        { row: 2, col: 3 },
        { row: 2, col: 4 },
      ],
      bridges: [{ row: 2, col: 2 }],
      food: [],
    },
    starThresholds: { three: 2, two: 3 },
    hint: 'Hop to the top side, then land on the flag square in one more jump.',
  },
{
  name: 'Horsin\' around ',
  description: 'So many hops, try not to backtrack!',
  pieceType: 'knight',
  start: { row: 2, col: 1 },
  goal: { row: 2, col: 3 },
  obstacles: {
    fences: [],
    rivers: [
        { row: 0, col: 0 },
        { row: 0, col: 1 },
        { row: 0, col: 2 },
        { row: 1, col: 1 },
        { row: 3, col: 3 },
        { row: 4, col: 2 },
        { row: 4, col: 4 },
      ],
    bridges: [
        { row: 0, col: 1 },
        { row: 4, col: 3 },
      ],
    food: [
        { row: 1, col: 2 },
        { row: 2, col: 0 },
      ],
  },
  starThresholds: { three: 6, two: 7 },
  hint: '2 hops, then 2 apples, then 2 more hops to the finish!',
},
  {
    name: 'Knight Parade',
    description: 'A longer hopping path for your final adventure.',
    pieceType: 'knight',
    start: { row: 4, col: 1 },
    goal: { row: 0, col: 0 },
    obstacles: {
      fences: [
        { row: 3, col: 1, side: 'top' },
        { row: 1, col: 3, side: 'left' },
      ],
      rivers: [
        { row: 2, col: 1 },
        { row: 2, col: 3 },
      ],
      bridges: [],
      food: [],
    },
    starThresholds: { three: 3, two: 4 },
    hint: 'Look for a three-jump route: the first hop is tricky, it lands FAR from the goal!',
  },
  {
    name: 'First Steps',
    description: 'The Pawn marches straight toward the goal!',
    pieceType: 'pawn',
    start: { row: 4, col: 2 },
    goal: { row: 0, col: 2 },
    obstacles: {
      fences: [],
      rivers: [],
      bridges: [],
      food: [],
    },
    starThresholds: { three: 4, two: 5 },
    hint: 'From the back rank, a pawn can move one or two squares forward. March straight up!',
  },
  {
    name: 'Harvest Time',
    description: 'Food blocks the straight path — eat it diagonally!',
    pieceType: 'pawn',
    start: { row: 4, col: 2 },
    goal: { row: 0, col: 2 },
    obstacles: {
      fences: [],
      rivers: [],
      bridges: [],
      food: [
        { row: 3, col: 1 },
        { row: 3, col: 3 },
        { row: 2, col: 2 },
      ],
    },
    starThresholds: { three: 5, two: 7 },
    hint: 'Pawns eat food diagonally — step left or right to grab the apple, then keep going!',
  },
  {
    name: 'The Long Road',
    description: 'Food blocks the straight path — eat your way back on track!',
    pieceType: 'pawn',
    start: { row: 4, col: 2 },
    goal: { row: 0, col: 2 },
    obstacles: {
      fences: [],
      rivers: [],
      bridges: [],
      food: [
        { row: 3, col: 1 },
        { row: 2, col: 2 },
        { row: 1, col: 2 },
      ],
    },
    starThresholds: { three: 4, two: 6 },
    hint: 'Eat diagonally left, and then avoid getting blocked!',
  },
  {
    name: 'Feast or Famine',
    description: 'Eat the first apple to unlock the path forward.',
    pieceType: 'pawn',
    start: { row: 4, col: 1 },
    goal: { row: 0, col: 3 },
    obstacles: {
      fences: [],
      rivers: [],
      bridges: [],
      food: [
        { row: 3, col: 2 },
        { row: 2, col: 3 },
      ],
    },
    starThresholds: { three: 5, two: 7 },
    hint: 'Eat diagonally to shift your column, then eat again to reach the right side.',
  },
  {
    name: "Pawn's Puzzle",
    description: 'Fences, food, and a long journey — put it all together!',
    pieceType: 'pawn',
    start: { row: 4, col: 0 },
    goal: { row: 0, col: 2 },
    obstacles: {
      fences: [
        { row: 2, col: 1, side: 'top' },
      ],
      rivers: [],
      bridges: [],
      food: [
        { row: 3, col: 1 },
        { row: 2, col: 2 },
        { row: 1, col: 1 },
      ],
    },
    starThresholds: { three: 6, two: 8 },
    hint: 'Eat diagonally to shift right, watch for the fence, then grab the last apple to reach the goal.',
  },
  {
    name: 'One Step',
    description: 'The King moves just one square at a time — any direction!',
    pieceType: 'king',
    start: { row: 4, col: 2 },
    goal: { row: 3, col: 2 },
    obstacles: {
      fences: [],
      rivers: [],
      bridges: [],
      food: [],
    },
    starThresholds: { three: 1, two: 2 },
    hint: 'The King moves exactly one square. Any direction works — even diagonal!',
  },
  {
    name: 'Corner to Corner',
    description: 'The King can reach anywhere — one careful step at a time.',
    pieceType: 'king',
    start: { row: 4, col: 4 },
    goal: { row: 0, col: 0 },
    obstacles: {
      fences: [],
      rivers: [],
      bridges: [],
      food: [],
    },
    starThresholds: { three: 4, two: 6 },
    hint: 'Move diagonally — the King covers ground fastest by stepping corner to corner!',
  },
  {
    name: 'Fence Hop',
    description: 'A fence blocks the direct path. Find the way around!',
    pieceType: 'king',
    start: { row: 4, col: 2 },
    goal: { row: 0, col: 2 },
    obstacles: {
      fences: [
        { row: 2, col: 2, side: 'top' },
      ],
      rivers: [],
      bridges: [],
      food: [],
    },
    starThresholds: { three: 4, two: 6 },
    hint: 'The fence stops the King from going straight up through the middle. Try slipping around the side!',
  },
  {
    name: 'The River King',
    description: 'A river blocks the path — but there is a bridge somewhere!',
    pieceType: 'king',
    start: { row: 4, col: 2 },
    goal: { row: 0, col: 2 },
    obstacles: {
      fences: [],
      rivers: [
        { row: 2, col: 0 },
        { row: 2, col: 1 },
        { row: 2, col: 2 },
        { row: 2, col: 3 },
        { row: 2, col: 4 },
      ],
      bridges: [
        { row: 2, col: 1 },
      ],
      food: [],
    },
    starThresholds: { three: 4, two: 6 },
    hint: 'The river cuts across the whole board! Find the bridge and step across — then continue to the goal.',
  },
];
